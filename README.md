...
升级 2.1.11 后，“显示更多”不再被首屏落底逻辑误判。键盘弹出和收回都持续跟随真实可见视口，直到布局恢复稳定才解除避让，不再依赖某个系统的固定键盘动画时长。该逻辑同时适用于 iOS/iPadOS、Android 和 Windows 触屏/软键盘环境；浏览器已自行缩放可见区时不会二次上抬，贴底阅读时保持最后一条与输入区连续。请同步更新 UI 与服务端目录并重启 SillyTavern。
