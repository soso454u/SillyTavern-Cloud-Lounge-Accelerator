...
升级 2.1.8 后，预设条目无论是否正在生成都会即时切换当前行，不再等待 PromptManager 重建完整列表；触屏设备的聊天输入区会跟随 `visualViewport` 稳定停在软键盘上方。交互自愈同时覆盖不可见但仍 open 的 modal，以及失去对应内容的 SillyTavern 旧式页面遮罩。请同步更新 UI 与服务端目录并重启 SillyTavern。
