...
升级 2.1.9 后，手机键盘避让不再逐帧缩短整个聊天布局，而是只用合成层 `transform` 平滑移动输入区；聊天背景会继续铺到输入法工具条后方，不再露出黑色空层。交互自愈继续静默工作，但不再弹出“已恢复输入焦点”等 toastr 通知。请同步更新 UI 与服务端目录并重启 SillyTavern。
