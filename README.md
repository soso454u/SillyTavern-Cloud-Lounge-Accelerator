...
升级 2.1.6 后，“界面操作优化”新增全平台交互自愈：精确恢复卡住的 closing popup、孤儿 loader 和用户明确点按失败的聊天输入焦点，并记录 blocker 诊断；正常弹窗、活动加载器和其他输入框不会被清理或抢焦点。手机普通顶栏抽屉不再动画 `height`，改为一次布局后的 `transform + opacity`，且打开期间持续关闭实时毛玻璃。请同步更新 UI 与服务端目录并重启 SillyTavern。
