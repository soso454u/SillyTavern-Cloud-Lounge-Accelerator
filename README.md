...
- 触屏设备只在当前顶栏抽屉开关动画期间临时停用重型模糊；iPhone/iPad 使用 90ms 快收，其他触屏使用 110ms，桌面继续使用官方动画。
...
升级 2.1.3 后再次运行一键安装器，根目录中旧的 `config.yaml.backup-cloud-lounge-*` 会自动整理：最早一份成为永久基线，最近且内容不同的最多 3 份成为滚动快照，迁移成功后才清理旧式文件；用户自己命名的其他备份不会被处理。移动端同时启用顶栏快收与更轻量的弹窗监听。请同步更新 UI 与服务端目录并重启 SillyTavern。
