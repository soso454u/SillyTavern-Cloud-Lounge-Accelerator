...
升级 2.0.8 后，弱网返回欢迎页会先显示可恢复的后台读取状态；同一页面已经成功读取过的最近聊天会立即复用，并在后台刷新。切换进入角色或群聊会强制校准到最后一条，同时让 SillyTavern 官方重算 Swipe 小三角。UI 仍会在注册 Worker 前核对服务端插件版本；请同步更新两个目录并重启 SillyTavern。
