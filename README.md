...
升级 2.1.2 后，iOS 主屏幕独立 Web App 检测到 Basic Auth 会进入兼容模式：自动注销旧 Worker、清理本插件缓存并停止重新注册，但不会关闭启动、聊天、正则或交互优化。版本签名改由服务端健康接口提供，不再额外 `HEAD /` 触碰认证根路径。请同步更新 UI 与服务端目录并重启 SillyTavern。
