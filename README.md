...
让云端 SillyTavern 打开更快、长聊天更流畅，预设、正则和世界书的操作也更顺手。安装后大部分功能都会自动工作，不需要手动调整复杂参数。
...
- 检测到完整 HTML 页面时，电脑只先显示最近 5 条，手机或低性能设备只先显示最近 3 条。
- 接近聊天顶部时，自动小批量补载旧消息并保持当前阅读位置。
- 长聊天或复杂美化聊天会自动减少屏幕外内容带来的负担。
- 最近代码优先高亮；旧代码进入视口后再处理，并默认以可点击预览形式折叠。
- 巨大的完整 HTML 源码会跳过语法高亮，避免在最终美化出现前重复处理大量节点。
...
- 预设面板先显示内容，Token 数在后台更新。
- 预设、正则和世界书拖动时使用轻量影子，减少复杂列表反复重排造成的卡顿。
- 松开后才提交一次最终顺序，并继续使用 SillyTavern 原有的保存功能。
- 复杂列表里的展开和折叠更轻快。
- 当前版本不支持某项优化时，会自动保留 SillyTavern 原来的操作方式。
...
升级 2.0.1 后，旧版 Worker 资源缓存会在激活阶段自动清理。旧设置会迁移为三个开关；旧版曾保存的聊天截断原值会优先恢复，再由自动算法接管当前会话。
