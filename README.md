...
升级 2.1.4 后，“界面操作优化”会自动选择桌面、触屏或 WebKit 档位；桌面端现在也获得顶栏快收、Popup 动画降重和拖动期间临时降重。实现只读取官方 UI 状态，不重写 drawer、popup 或 sortable，并尊重系统“减少动态效果”。2.1.3 引入的有界配置备份与旧备份自动整理继续保留。请同步更新 UI 与服务端目录并重启 SillyTavern。
