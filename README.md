...
升级 2.0.6 后，UI 会在注册 Worker 前核对服务端插件版本。版本不一致时会自动注销旧 Worker 并清理本插件缓存；请同步更新服务端目录并重启 SillyTavern，刷新页面后即可恢复页面缓存。旧设置仍会迁移为三个开关，旧版曾保存的聊天截断原值会优先恢复，再由每页 5 条的聊天加载方式接管当前会话。
