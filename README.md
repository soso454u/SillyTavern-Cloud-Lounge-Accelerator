...
- 点击“显示更多”时由 SillyTavern 原生流程每页补载 5 条旧消息并保持阅读位置；插件不再接管滚动或二次补偿高度。
...
- 手机会看护 SillyTavern 原生 modal 关闭流程：只有已经失效的透明关闭/加载遮罩才会被自动解除，真正活动的加载任务保持阻塞。
- 快捷回复执行窗处于透明最小化状态时，点到背景会将控制窗展开，但不会擅自终止正在执行的脚本。
- 顶栏抽屉启用全平台自动档：桌面打开/关闭为 160/130ms，普通触屏为 150/110ms，iPhone/iPad 为 130/90ms；只在当前抽屉动画期间临时停用重型模糊。
...
升级 2.1.5 后，“界面操作优化”会在运行时识别 SillyTavern #5819 的旧版顶栏 `:has()` 规则，只把它限制到官方桌面断点；手机不再因打开普通顶栏抽屉而持续触发整页样式失效。官方修复后此补丁会自动跳过，关闭优化时也会恢复原规则。抽屉动画期间不再添加无效的 `will-change: height`，桌面、触屏和 WebKit 原有分档仍然保留。请同步更新 UI 与服务端目录并重启 SillyTavern。
