...
test('keeps every published version source in sync', async () => {
    ...
    assert.equal(CLIENT_VERSION, '2.1.21');
...
test('pauses chat scanning during generation and leaves native sortable ownership untouched', async () => {
    ...
    assert.doesNotMatch(interactionSource, /KeyboardOverlayGuard|MobileViewportGuard|visualViewport/);
    ...
    assert.doesNotMatch(styles, /cla-keyboard-overlay|--cla-keyboard-inset|--cla-keyboard-shift/);
